<template>
  <div>
    <v-snackbar right top :value="true" :color="snackbarData.color" timeout="2000" v-model="snackbar" shaped>
    ...
    <v-container v-if="userProfile" class="mt-5">
      <v-card max-width="1000px" class="mx-auto bg" elevation="2">
        <v-card-title>Profile Detail</v-card-title>
        <v-img height="200px" :src="user.coverImage"
        ...
        <div class="d-flex justify-center mt-5">
          <v-btn outlined color="#cb3744" @click="addToPhone">
            Add to Phone
          </v-btn>
          <a :href="openVcf"></a>
          <!-- <v-btn
        ...
        <v-row color="#0000" class="profile-text-name ma-4 pt-16">
          <v-col v-for="(item, i) in userProfileData" :key="i" cols="6">
            <label for=""><strong>{{ item.label }}</strong></label>
            <div v-if="item.label != 'Website' && item.label != 'Linkedin' && item.label != 'Whatsapp number'"
              class="mt-2 font-12">
              {{ userProfile[item.identifier] }}
            </div>
            <span v-if="
              item.identifier == 'website' || item.identifier == 'Linkedin'
            ">
              <a :href="userProfile[item.identifier]">
                <div>
                  {{ userProfile[item.identifier] }}
                </div>
              </a>
            ...
            <span v-if="
              item.identifier == 'mobile_number'
            ">
              <a :href="`https://wa.me/${userProfile[item.identifier]}`">
                <div>
                  {{ userProfile[item.identifier] }}
                </div>
              </a>
  ...
  methods: {
      ...
      axios
        ...
        .then((res) => {
          ...
          this.user = {
            ...
            image: `${axios.defaults.baseURL}${res.data.image}`,
            coverImage: `${axios.defaults.baseURL}${res.data.coverImage}`
          };
        })
        .catch((err) => {
          this.snackbar = true;
    ...
    addToPhone() {
      axios({
        ...
        let vcard = "data:text/x-vcard;charset=utf-8;base64,";
        this.openVcf = vcard + response.data;
        var docUrl = document.createElement("a");
        docUrl.href = this.openVcf;
